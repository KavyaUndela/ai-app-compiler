...
export default function LandingPage() {
  ...
  const activeResult = useMemo(() => result, [result])

  async function handleGenerate() {
    setLoading(true)
    try {
      const res = await postGenerate({ requirement_text: prompt })
      setResult(res)
    } catch (e) {
      ...
      <header className="relative border-b border-white/5 bg-slate-950/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          ...
          <nav className="hidden items-center gap-2 md:flex">
            <Link href="/pipeline" className="rounded-full px-4 py-2 text-sm text-slate-300 transition hover:bg-white/5 hover:text-white">Pipeline Viewer</Link>
            <Link href="/validation" className="rounded-full px-4 py-2 text-sm text-slate-300 transition hover:bg-white/5 hover:text-white">Validation Viewer</Link>
            <Link href="/repair" className="rounded-full px-4 py-2 text-sm text-slate-300 transition hover:bg-white/5 hover:text-white">Repair Viewer</Link>
            <Link href="/runtime" className="rounded-full px-4 py-2 text-sm text-slate-300 transition hover:bg-white/5 hover:text-white">Runtime Preview</Link>
          </nav>
        </div>
      </header>
        ...
        <div className="grid gap-8 xl:grid-cols-[1.08fr_0.92fr] xl:items-start">
          <div className="space-y-8">
            ...
            <div className="flex flex-wrap gap-4">
              <a href="#composer" className="inline-flex items-center justify-center rounded-full bg-cyan-500 px-6 py-4 text-sm font-semibold text-slate-950 transition hover:bg-cyan-400">Start from prompt</a>
              <Link href="/pipeline" className="inline-flex items-center justify-center rounded-full border border-white/10 bg-white/5 px-6 py-4 text-sm font-semibold text-slate-200 transition hover:bg-white/10 hover:text-white">View pipeline</Link>
            </div>

            <div className="grid gap-4 sm:grid-cols-3">
              <div className="rounded-3xl border border-white/8 bg-white/5 p-5 shadow-xl shadow-black/10 backdrop-blur">
                <div className="text-3xl font-semibold text-white">6</div>
                <p className="mt-2 text-sm text-slate-400">deterministic stages</p>
              </div>
              <div className="rounded-3xl border border-white/8 bg-white/5 p-5 shadow-xl shadow-black/10 backdrop-blur">
                <div className="text-3xl font-semibold text-white">JSON</div>
                <p className="mt-2 text-sm text-slate-400">structured outputs</p>
              </div>
              <div className="rounded-3xl border border-white/8 bg-white/5 p-5 shadow-xl shadow-black/10 backdrop-blur">
                <div className="text-3xl font-semibold text-white">Live</div>
                <p className="mt-2 text-sm text-slate-400">runtime previews</p>
              </div>
            </div>
          ...
          <section id="composer" className="rounded-[2rem] border border-white/8 bg-white/5 p-6 shadow-2xl shadow-black/20 backdrop-blur-xl sm:p-8">
            ...
            <div className="mt-6 space-y-4">
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                ...
                <Link href="/runtime" className="inline-flex items-center justify-center rounded-full border border-white/10 bg-white/5 px-6 py-3 text-sm font-semibold text-slate-200 transition hover:bg-white/10 hover:text-white">Runtime preview</Link>
              </div>
            </div>
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              <div className="rounded-3xl border border-white/8 bg-slate-950/80 p-5">
                ...
                <div className="text-2xl font-semibold text-cyan-300">{activeResult ? 'Ready' : 'Ready to generate'}</div>
                <p className="mt-3 text-sm text-slate-400">{activeResult ? 'Result loaded below.' : 'The compiler will create intent, schemas, validation, repair, and runtime output.'}</p>
              </div>
              <div className="rounded-3xl border border-white/8 bg-slate-950/80 p-5">
                <div className="mb-3 text-xs uppercase tracking-[0.2em] text-slate-500">Result preview</div>
                <div className="rounded-2xl border border-white/8 bg-slate-900/80 p-4 text-sm text-slate-300">
                  <pre className="max-h-56 overflow-auto whitespace-pre-wrap">{activeResult ? JSON.stringify(activeResult, null, 2) : 'No result yet.'}</pre>
                </div>
              </div>
            </div>
          </section>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-[2rem] border border-white/8 bg-slate-900/60 p-6">
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                {pipelineSteps.map((step) => (
                  <div key={step} className="rounded-2xl border border-white/8 bg-slate-950/70 px-3 py-3 text-xs font-medium text-slate-200">{step}</div>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-[2rem] border border-white/8 bg-slate-900/60 p-6">
              ...
              <div className="mt-4 grid gap-3">
                <Link href="/pipeline" className="rounded-2xl border border-white/8 bg-slate-950/70 px-4 py-4 text-sm text-slate-200 transition hover:border-cyan-500/40">Pipeline Viewer</Link>
                <Link href="/validation" className="rounded-2xl border border-white/8 bg-slate-950/70 px-4 py-4 text-sm text-slate-200 transition hover:border-cyan-500/40">Validation Viewer</Link>
                <Link href="/repair" className="rounded-2xl border border-white/8 bg-slate-950/70 px-4 py-4 text-sm text-slate-200 transition hover:border-cyan-500/40">Repair Viewer</Link>
                <Link href="/runtime" className="rounded-2xl border border-white/8 bg-slate-950/70 px-4 py-4 text-sm text-slate-200 transition hover:border-cyan-500/40">Runtime Preview</Link>
              </div>
            </div>
          </div>
        </section>
